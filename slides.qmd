...
Gutenberg, Bibliothèque électronique du Québec, Github
...
pdf = pdf.dropna(subset=["avg_words", "avg_sentences"])
...
if has_zipf:
    ...
    sample = zipf_pdf.merge(first_ids, on="author")
    sample = sample[sample["id"] == sample["first_id"]].copy()
    sample = sample.sort_values("frequency", ascending=False)
    sample = sample.groupby("author").head(50)
    ...
    fig_scatter = px.scatter(
        ...
        y="log_freq",
        color="author",
        text="word",           # label = le mot lui-même
        title="Zipf log-log — mots caractéristiques par auteur (1 texte représentatif)",
        labels={
            "log_rank": "log₁₀(rang)",
            "log_freq": "log₁₀(fréquence)",
            "author"  : "Auteur"
        },
    )
    ...
    fig_scatter.update_layout(
        template="plotly_white",
        xaxis_title="log₁₀(rang du mot)",
        yaxis_title="log₁₀(fréquence du mot)",
        legend_title="Auteur",
    )
    fig_scatter.show()
```
